...
class Ogre implements Runnable {
    ...
    public synchronized void rebreDany(int dany, String nom){
        if(vida <= 0){
            System.out.println("El ogro ha muerto. Los heroes han ganado");
            return;
        }
        vida -= dany;
        if (vida < 0) vida = 0;
    ...
    public void run(){
        while(estaViu()){
            ...
            Heroi defensor = herois.get(rnd.nextInt(herois.size())); // Saca un numero random en base al tamaño del array de los heroes y ese Index lo usa para atacar
            if(defensor.estaViu()){
                defensor.rebreAtac(20 + rnd.nextInt(21));
            }
...
abstract class Heroi implements Runnable{
    ...
    public synchronized boolean estaViu(){ // Esto es syncronized pq tambien tiene que poder acceder la Sanadora
        return vida > 0;
    }
    ...
    public synchronized void rebreAtac(int dany){
        ...
        vida -= dany;
        if (vida <= 0) { // Si el daño es mortal
            vida = 0; // ( esto es por si hace overkill)
            System.out.println(nom + " cae KO y espera a Eylin"); // Muere

            try {
                wait(); // Espera a ser revivido
            } catch (InterruptedException e) {}

            System.out.println(nom + " ha sido revivido y vuelve al combate"); // Revive
        } else {
